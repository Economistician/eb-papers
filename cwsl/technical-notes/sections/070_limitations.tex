...
interpretability depends on selecting values that reflect the operational
priorities assumed by the evaluation. These weights encode policy or planning
preferences rather than objective or universally correct economic costs. If
cost weights are misaligned with system priorities, the resulting \CWSL{}
values may misrepresent relative risk or readiness.
...
\subsection{Dependence on Data Admissibility and Evaluation Scope}

\CWSL{} assumes that the evaluated observations are valid, comparable, and
representative of the operational context of interest. Inclusion of inadmissible
intervals—such as periods affected by outages, closures, censored demand, or
data quality issues—may distort the resulting metric values. Defining the
appropriate evaluation scope and ensuring data admissibility are therefore
prerequisites for meaningful interpretation of \CWSL{}.
